...
impl Handle {
    pub fn invoke(&self, method: &str, args: &[Variant]) -> Result<Option<Variant>, &'static str> {
        unsafe {
            ...
            let c_args = ffi::rsqml_varlist_create();
            assert!(!c_args.is_null());
            for arg in args.iter() {
                let c_arg = ffi::rsqml_varlist_push(c_args);
                assert!(!c_arg.is_null());
                arg.to_qvariant(c_arg);
            }

            let result = ffi::rsqml_variant_create();
            assert!(!result.is_null());

            match self.i.upgrade() {
                Some(i) => ffi::rsqml_engine_invoke(i.p, cstr.as_ptr(), result,
                                               c_args as *const QVariantList),
                None    => {
                    ffi::rsqml_variant_destroy(result);
                    ffi::rsqml_varlist_destroy(c_args);
